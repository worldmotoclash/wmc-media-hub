        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="text-xs text-muted-foreground border-t pt-4">
              <span>Salesforce ID: <a href={`https://worldmotoclash.my.salesforce.com/${(video.salesforceId || video.id).replace(/^sf_/, '')}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 underline">{(video.salesforceId || video.id).replace(/^sf_/, '')}</a></span>
            </div>
          </div>
        </div>
      </DialogContent>
